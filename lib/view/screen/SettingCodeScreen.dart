...
class SettingCodeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Set<int> codeItemIdSet = _memberController.memberCodeList
        .map((element) => element!.codeItemId)
        .toSet();
    _codeController.setSelectedItemIdSet(codeItemIdSet);
          ...
          children: [
                      ...
                      children: [
                        ...
                        GridView.builder(
                          ...
                          itemBuilder: (context, itemIndex) {
                            ...
                            return Obx(() {
                              final isSelected = _codeController.isSelected(item.id);
                              return GestureDetector(
                                onTap: () => _codeController.toggleItemSelect(item.id),
                                child: Column(
                                  children: [
                                    // 이미지
                                    Expanded(
                                      child: Container(
                                        decoration: BoxDecoration(
                                          ...
                                          image: DecorationImage(
                                            image: AssetImage('assets/images/minji.jpg'),
                                            fit: BoxFit.cover,
            ...
            Obx(() {
              ...
              return ElevatedButton(
                onPressed: () {
                  // 선택된 아이템들 저장 로직
                  if (_codeController.selectedItemIdSet.length < 3) {
                    Get.dialog(
                      AlertDialog(
                        title: Text('최소 선택 필요'),
                    ...
                    _memberController.updateMemberCodeMap();

                    Get.closeAllSnackbars();
                    Get.back();
                    Get.snackbar(
                        '관심사 저장',
                        '${_codeController.selectedItemIdSet.length}개의 관심사가 저장되었습니다.',
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  backgroundColor: _codeController.selectedItemIdSet.length >= 3
                      ? Colors.orange
                      : Colors.grey,
                ),
                child: Text(
                  '저장하기 (${_codeController.selectedItemIdSet.length}/10)',
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
